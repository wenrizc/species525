...
class CUBDataset(Dataset):
    def __init__(self, path, train=True, transform=None, target_transform=None):
        # 初始化函数
        self.root = path  # 数据集根目录
        self.is_train = train  # 是否是训练集
        self.transform = transform  # 图像变换
        self.target_transform = target_transform  # 标签变换
        self.images_path = {}  # 存储图像路径的字典
        with open(os.path.join(self.root, 'images.txt')) as f:
            for line in f:
                image_id, path = line.split()
                self.images_path[image_id] = path  # 读取图像路径

        self.class_ids = {}  # 存储图像类别的字典
        with open(os.path.join(self.root, 'image_class_labels.txt')) as f:
            for line in f:
                image_id, class_id = line.split()
                self.class_ids[image_id] = class_id  # 读取图像类别

        self.data_id = []  # 存储数据ID的列表
        with open(os.path.join(self.root, 'train_test_split.txt')) as f:
            for line in f:
                image_id, is_train = line.split()
                if int(is_train) == self.is_train:
                    self.data_id.append(image_id)  # 读取训练或测试数据ID
    ...
    def __getitem__(self, index):
        ...
        Returns:
            图像及其对应的标签
        """
        image_id = self.data_id[index]  # 获取图像ID
        class_id = int(self._get_class_by_id(image_id)) - 1  # 获取图像类别
        path = self._get_path_by_id(image_id)  # 获取图像路径
        image = cv2.imread(os.path.join(self.root, 'images', path))  # 读取图像

        if self.transform:
            image = self.transform(image)  # 应用图像变换

        if self.target_transform:
            class_id = self.target_transform(class_id)  # 应用标签变换
